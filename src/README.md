...
The `restartDevices` method also exists on [`Location`](https://doc.deno.land/https/deno.land/x/jamf_school@0.4.1/mod.ts#Location) and [`DeviceGroup`](https://doc.deno.land/https/deno.land/x/jamf_school@0.4.1/mod.ts#DeviceGroup) as a convenience for calling `getDevices` and restarting them in parallel. See the implementation in the relevant file in [./internal](./internal).
...
- **Added `Device.enrollment` ([#53](https://github.com/SeparateRecords/deno_jamf_school/issues/53))** <br>
  It's an object instead of a string. See the docs for more information.

- **Added `Client.getUserByName` ([#53](https://github.com/SeparateRecords/deno_jamf_school/issues/53))** <br>
